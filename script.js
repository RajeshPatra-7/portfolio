(() => {
  ...
  const setLoadingProgress = (value) => {
    ...
    loadingValue.textContent = String(safeValue);
  };

  const finishLoading = (delay = 500) => {
    setLoadingProgress(100);
    if (!loadingScreen) return;
    window.setTimeout(() => loadingScreen.classList.add("hidden"), delay);
  };

  const failBoot = (message) => {
    console.error(message);
    if (loadingTitle) loadingTitle.textContent = "EXPERIENCE LOAD WARNING";
    finishLoading(1200);
  };

  const toRepoRelativePath = (path) => {
  ...
  const normalizePortfolioPaths = (portfolio) => ({
    ...portfolio,
    heroImage: toRepoRelativePath(portfolio.heroImage),
    projects: (portfolio.projects || []).map((project) => ({
      ...project,
      image: toRepoRelativePath(project.image),
      video: toRepoRelativePath(project.video)
    }))
  });

  const dependencyErrors = [];
  if (!window.THREE) dependencyErrors.push("Three.js failed to load.");
  if (!window.gsap) dependencyErrors.push("GSAP failed to load.");
  if (!window.ScrollTrigger) dependencyErrors.push("GSAP ScrollTrigger failed to load.");

  const rawPortfolio = window.PORTFOLIO;
  if (!rawPortfolio || !Array.isArray(rawPortfolio.projects)) {
    dependencyErrors.push("Portfolio data is missing or invalid.");
  }

  if (dependencyErrors.length > 0) {
    failBoot(dependencyErrors.join(" "));
    return;
  }

  const portfolio = normalizePortfolioPaths(rawPortfolio);
  const texturePaths = [portfolio.heroImage, ...portfolio.projects.map((project) => project.image)].filter(Boolean);
  const fallbackTexture = buildFallbackTexture(window.THREE);
  ...
  preloadTextures(texturePaths)
    .then(() => {
      ...
      startExperience(portfolio, textureCache, fallbackTexture);
      finishLoading(350);
    })
    .catch((error) => {
      failBoot(`Texture preload failed: ${error?.message || error}`);
      startExperience(portfolio, textureCache, fallbackTexture);
    });
  ...
  function preloadTextures(paths) {
      ...
      new Promise((resolve) => {
        loader.load(
          ...
          (texture) => {
            ...
            loaded += 1;
            setLoadingProgress(5 + (loaded / uniquePaths.length) * 85);
            resolve();
          },
          undefined,
          () => {
    ...
    const timeout = new Promise((resolve) => {
      window.setTimeout(() => {
        console.warn("Texture preload timeout reached; continuing with available assets.");
        resolve();
      }, 10000);
    });

    return Promise.race([Promise.all(uniquePaths.map(loadOne)), timeout]);
  }

  function buildFallbackTexture(THREE) {
  ...
  function startExperience(portfolioData, preloadedTextures, fallback) {
    try {
      ...
      document.getElementById("contact-email").href = `mailto:${portfolioData.social.email}`;
      ...
      const sceneNames = ["hero", ...portfolioData.projects.map(() => "projects"), "about", "skills", "contact"];
      let currentScene = -1;
      ...
      function createHeroGroup() {
        ...
        const frameMat = new THREE.MeshPhysicalMaterial({
          map: safeTexture(portfolioData.heroImage),
          roughness: 0.92,
          metalness: 0.06,
          clearcoat: 0.5
        });
      ...
      function createProjectGroups() {
        return portfolioData.projects.map((project, i) => {
          const group = new THREE.Group();
          ...
          video.element.addEventListener("error", () => {
            video.failed = true;
            console.warn(`Video failed to load: ${video.source}`);
          });
      ...
      function createAboutGroup(z) {
        const group = new THREE.Group();
        group.position.z = z;
        const words = portfolioData.aboutKeywords;
      ...
      function setActiveVideo(target) {
        projectGroups.forEach((entry, i) => {
          if (i === target) {
            if (entry.video.failed || !entry.video.source) return;
      ...
      function safeTexture(path) {
        ...
        return preloadedTextures.get(path) || fallback;
      }
      ...
      failBoot(`Experience initialization failed: ${error?.message || error}`);
    }
  }
})();
